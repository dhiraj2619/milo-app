...
import {ArrowLeft, EllipsisVertical, Paperclip, Phone, Send, Smile, Video} from 'lucide-react-native';
...
export default function ChatConversationScreen({navigation, route}) {
  ...
  const list = useRef();
  useEffect(() => {setMessages([{id: 'welcome', text: 'Hii', mine: true, time: now()}]);}, []);
  const sendMessage = () => {
    const text = draft.trim();
    if (!text) return;
    setMessages(current => [...current, {id: String(Date.now()), text, mine: true, time: now()}]);
    setDraft('');
  };
